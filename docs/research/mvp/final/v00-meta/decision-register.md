...
| **D4** | Overlapping-RFC1918 collision across N nets | **IPv6 ULA /48 per tenant + 4via6** | **CGNAT 100.64/10 1:1 per network** | **4via6** for scale + clean overlay; hide the mapping behind Console UX (a v1 must-decide) | doc 01 (routing) + doc 04/`svc-ipam.md` + an MVP multi-connector overlap test | 4via6 client/OS support proves inadequate on a target platform → fall back to documented per-network NAT (FR-303 already provisions it) |
