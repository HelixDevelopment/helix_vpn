...
| G2 MASQUE through DPI block | ☐ pass ☐ fail | goodput @5% loss …, tshark … | — |
| G3 iOS memory | ☐ pass ☐ fail | peak RSS … / ceiling … / headroom … % | core stays Rust ↔ fallback §6.4 |
| G4 edge language | ☐ Rust ☐ Go | CPU/Gbps R:… G:…; p99 R:… G:…; MASQUE effort … | single-impl ↔ dual-impl |
| G5 FFI | ☐ pass ☐ fail | demo recording | frb v2 ↔ UniFFI |
| G6 reconcile | ☐ pass ☐ fail | map-delta → reachable, no restart | confirms push model |
