...
| G5 FFI | ☑ pass | `submodules/helix_shims/crates/helix-ffi` (branch `feature/hvpn-p0-049-ffi-surface`, commit `4958072`): `cargo test --all-targets` 9/9 passed (independently re-verified by controller) — real connector rig + real `map.json` + real WireGuard handshake via `helix-orch`; ordered `Connecting → Handshaking → Connected{transport,rtt_ms}` observed through `status_stream`; idempotent `stop()` + restart proven. Dart/Flutter codegen deferred — no `dart`/`flutter` toolchain in this environment (confirmed via `flutter_rust_bridge_codegen`'s own explicit failure, not a defect in the Rust surface); `helix-masque` not yet wired into `wg_session` at the orchestrator layer, so `transport` config is accepted but the dial always uses plain-UDP for now. **2026-07-07 Phase-1-rigor hardening (`HVPN-P1-005`, `helix_shims` commit `a323196`, independently re-verified by controller post-merge):** test suite grown 14→23. UI-contract proven: late `status_stream()` subscription replays full history, concurrent subscribers see a consistent ordered view, a dropped subscriber is pruned without disrupting others, concurrent `start()` resolves to exactly one winner, concurrent `stop()` is idempotent with exactly one terminal event. E2E extended to 3 full connect/disconnect/reconnect cycles. Real positive finding: a stopped session's sink genuinely closes (maps to Dart's `onDone`) rather than merely falling silent — the test was corrected to assert the real behavior instead of an assumption. Honest scope note: no Dart/Flutter toolchain in this sandbox, so "UI" was tested as the UI-facing contract `status_stream()` exposes, not a fabricated Flutter harness. | frb v2.12.0 confirmed viable; Rust-side surface complete; Dart bindings + Flutter demo recording tracked as a follow-up task |
